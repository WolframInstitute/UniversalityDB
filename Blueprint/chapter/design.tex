...
\begin{enumerate}
\item \textbf{What is a vertex?} A \emph{system family} with possibly
  free parameters. Specific machines are families with singleton parameter space.
\item \textbf{Quantification?} Carried on edges, not vertices. Each edge
...
\item \textbf{Verification granularity?} Four levels:
  \texttt{FullyVerified} (no sorry),
  \texttt{SpotChecked} (native\_decide),
  \texttt{TypeChecked} (sorry proof),
  \texttt{Unformalized} (paper only).
\end{enumerate}
